...
**最后更新：2026 年 4 月 2 日**（含架构章节整合；**§11.0 立即下单** 与 Member/VIP/Merchant 权限说明）

### 架构与变更纪律（必读）

本仓库是**单仓多应用**：**Admin、三 Web、三 App** 共用 **Supabase**，部署上又分**根目录 Admin** 与**各子目录 Netlify 站点**、**各 Expo 工程**。本文 **§1–§7、§3、§9、§11** 等记录的是**长期应遵守的边界**（五端谁建在哪、数据从哪来、敏感能力走哪条 Serverless、业务规则如身份与支付等）。

**以后任何需求、重构或加功能，在动手前都应对照本文档**，避免误动：子项目目录、站点 `publish` 与 `deploy:netlify` 的 **site id**、**勿混用**的 `netlify/functions`、数据库/迁移与 RLS 前提、以及跨端要一致的业务（如 **§11.0**）。**架构、部署边界或五端数据流改错**往往表现为单端「能跑」、别端/对账/生产环境异常，排查与回滚**非常费时**；宁可先小步对照文档、再改代码。
...
| §10 | 路由与页面索引（Web） |
| §11 | 核心页面与业务摘要（含 **§11.0 立即下单** 与身份权限） |
| §12 | 开发与部署注意事项 |
...
### 11.0 立即下单与账号身份（Member / VIP / Merchant）

**背景**：多终端的「立即下单」流程会按用户 **`userType`** 区分能力（同城商场、跑腿费/商品费支付方式、商家 COD 等）。下列为**产品规则摘要**，若与线上校验或 UI 不一致，以**当前端内实现**为准，并应同步本段。

| 能力维度 | **Member** | **VIP** | **Merchant** |
|----------|------------|---------|--------------|
| **同城商场** | 不能从「同城商场」购物 | 可以 | 不可以从「同城商场」下单（商家场景不走商城路径） |
| **跑腿费** | 只能用 **现金**；**不能**用余额 | 可以用 **余额** | **不能**用余额，仅 **现金** |
| **商品费** | **不能**用现金支付 | 可以用 **余额**；**不能**用现金支付 | 见下：COD 可开关 |
| **商家卖货 / 代收** | — | — | 可开 **COD**，由骑手代收尾款；也可关 COD、**不**收商品费 |
...
- **Merchant**
  1. 不可以从「同城商场」中下单。
  2. 向客户销售自有商品时，可启用 **COD**，由骑手帮忙代收尾款。
  3. 也可关闭 COD，要求骑手**不**代收商品费。
  4. 不能使用「余额支付」支付**跑腿费**（仅可现金支付跑腿费）。

> **实现提示**：`ProfilePage`（商家端 Web 等）与下单页上的支付方式、商场入口、COD 开关等需与本规则对齐；改规则时各端与 Admin 对账展示一并评估。
...
1. **架构与文档先行**：大改、加端、换域名/站点、动订单或支付/权限前，**先**核对 **本指南对应章节** 与 `NETLIFY_DOMAIN_CONFIG.md` 等，避免和既有约定冲突（详见文首**架构与变更纪律**）。
2. 巨型 `ProfilePage.tsx` 等改后运行 `build` 或 CI，避免未闭合标签。
3. **ESLint**：CRA 可能仅 warning；长期应消除；临时可用 `DISABLE_ESLINT_PLUGIN=true`（不推荐长期）。
4. **移动端发版**：**§13** 必读；**骑手端** 无提交原生时以 `app.json` 与 `eas.json` 的 `autoIncrement` 策略为准。
5. **隐私与商店**：对外 URL 与 C 站一致（如 `https://market-link-express.com/privacy-policy`）。
6. **Sentry**（部分 Web）：`REACT_APP_SENTRY_DSN` + 客户端 `LoggerService` 或 error service。
7. **Git 安全**：不在远程 URL 中嵌入个人 token；用 SSH/凭据管理。
8. **GitHub Release**：`/download` 若指向固定 APK 文件名，发版时附件必须存在（勿依赖空的 `latest` 附言）。
...
**维护约定**：子项目增删、换路由、增 Netlify 函数、换生产域名、换 Expo 策略时，**同步更新** 本文 **§1–§7、§3、§9、§10** 中对应表或段落。若**立即下单**的支付方式、商场或 COD 规则有变更，**同步更新 §11.0**。**改架构或边界时切忌只改单端、单文件**；与本文不一致的提交往往会在其他端或线上才暴露，后续排障成本高。

---

*本文档为《MARKET LINK EXPRESS 单仓架构》的单一入口；与具体 PR 冲突时，以当前仓库内源码与 `netlify.toml` / `app.json` 为准。新改动请持续对照**文首**与 **§12 第 1 条**所强调的架构纪律，避免「改错架构、全线埋雷」。*
