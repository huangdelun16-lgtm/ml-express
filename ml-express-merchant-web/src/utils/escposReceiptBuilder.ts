...
function appendBytes(bytes: number[], chunk: Uint8Array): void {
...
function appendLine(bytes: number[], text: string, maxChars: number): void {
  for (const line of wrapText(text, maxChars)) {
    appendBytes(bytes, asciiBytes(line));
    bytes.push(LF);
  }
}

function appendSeparator(bytes: number[], maxChars: number): void {
  appendBytes(bytes, asciiBytes('-'.repeat(Math.max(8, Math.min(maxChars, 48)))));
  bytes.push(LF);
}

function appendKeyValue(bytes: number[], label: string, value: string, maxChars: number): void {
...
export function buildEscPosReceiptBytes(
  ...
  const createdLabel = toEscPosSafeText(new Date(data.createdAt).toLocaleString('en-GB'));
  const bytes: number[] = [];
  ...
  if (data.isSample) appendLine(bytes, ESCPOS_RECEIPT_LABELS.sample, maxChars);
  bytes.push(ESC, 0x45, 0x01);
  ...
  appendLine(bytes, `${ESCPOS_RECEIPT_LABELS.addr}: ${toEscPosSafeText(data.receiverAddress || '-')}`, maxChars);
  appendSeparator(bytes, maxChars);
  appendKeyValue(bytes, ESCPOS_RECEIPT_LABELS.pay, paymentText, maxChars);

  for (const display of buildReceiptItemDisplays(data.items, itemLabelForEscPos)) {
    appendLine(bytes, formatReceiptItemEscPosLine(display, maxChars), maxChars);
  }

  appendKeyValue(bytes, ESCPOS_RECEIPT_LABELS.delivery, `${data.deliveryFee.toLocaleString()} MMK`, maxChars);
  bytes.push(ESC, 0x45, 0x01);
...
export function escPosBytesToBase64(bytes: Uint8Array): string {
  let binary = '';
