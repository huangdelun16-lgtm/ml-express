-- 装车出库车次：RUI0001 / MUS0001 等，同一次装车多包共享同一车次

ALTER TABLE inventory_pkg_tracking
  ADD COLUMN IF NOT EXISTS trip_number TEXT;

ALTER TABLE inventory_packed_shipments
  ADD COLUMN IF NOT EXISTS trip_number TEXT;

CREATE INDEX IF NOT EXISTS idx_inventory_pkg_tracking_trip_number
  ON inventory_pkg_tracking (trip_number)
  WHERE trip_number IS NOT NULL AND trip_number <> '';

CREATE INDEX IF NOT EXISTS idx_inventory_pkg_tracking_origin_trip
  ON inventory_pkg_tracking (origin_store_code, trip_number, truck_loaded_at DESC);

CREATE TABLE IF NOT EXISTS inventory_trip_sequences (
