-- 多个入库：订单直接以「已入库 + 已打包、库存 0」写入，整包重量只在快递包上。
...
BEGIN
  ...
  END IF;
  IF upper(trim(coalesce(p_payload->>'store_code', ''))) <> v_store_code THEN
...
REVOKE ALL ON FUNCTION inventory_packaging_stock_in_batch(UUID, JSONB) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION inventory_packaging_stock_in_batch(UUID, JSONB) TO authenticated;
