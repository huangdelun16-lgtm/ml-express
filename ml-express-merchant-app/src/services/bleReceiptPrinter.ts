...
import { buildEscPosReceiptBytes } from '../utils/escposReceiptBuilder';
import type { MerchantReceiptData } from '../utils/merchantReceiptTemplate';
...
async function characteristicExists(device: Device, target: BleWriteTarget): Promise<boolean> {
  try {
    ...
    return chars.some(
      (item) => normalizeUuid(item.uuid) === normalizeUuid(target.characteristicUUID),
    );
  } catch {
    return false;
  }
...
export async function sendEscPosPayload(bytes: Uint8Array): Promise<void> {
...
export async function printMerchantReceiptViaBle(
  data: MerchantReceiptData,
  paperWidthMm: ReceiptPaperWidthMm,
): Promise<void> {
  const payload = buildEscPosReceiptBytes(data, paperWidthMm);
  await sendEscPosPayload(payload);
}
