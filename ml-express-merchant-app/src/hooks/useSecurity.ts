...
export const useSecurity = () => {
  ...
  useEffect(() => {
    const checkSecurity = async () => {
      try {
        ...
        LoggerService.error('Security check failed:', error);
        setIsSecure(false);
  ...
  const secureStore = useCallback(async (key: string, data: string, type: SensitiveDataType) => {
    try {
      ...
      LoggerService.error('Secure store failed:', error);
      return false;
    }
  }, []);
  // 安全获取数据
  const secureGet = useCallback(async (key: string, type: SensitiveDataType) => {
    try {
      ...
      LoggerService.error('Secure get failed:', error);
      return null;
    }
  }, []);
  // 安全删除数据
  const secureRemove = useCallback(async (key: string) => {
    try {
      ...
      LoggerService.error('Secure remove failed:', error);
    }
  }, []);
  // 生成安全令牌
