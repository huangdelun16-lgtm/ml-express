...
function computeOrderStats(rows: Array<{ id?: string; status?: string }>) {
  return {
    ...
    awaitingPickup: rows.filter((p) => p.status === '待取件').length,
    packing: rows.filter((p) => p.status === '打包中').length,
    pickedUp: rows.filter((p) => p.status === '已取件').length,
    ...
    shipping: rows.filter((p) => p.status === '配送中').length,
    deliveredReady: rows.filter((p) => p.status === '已送达').length,
