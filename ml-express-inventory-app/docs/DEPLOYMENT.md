...
当前客户端版本：**v1.9.1 (24)**。Inventory App 为在线专用，Supabase 是唯一业务数据源。
...
每次发布新 APK 后只需更新 `system_settings` 中 `inventory.android.latest_release` 一条记录，**无需**重新发版 App 即可让旧版用户看到更新提示（旧版 App 需已包含此功能，自 v1.6.0 起；当前 **v1.9.1 (24)**）。
...
先确认两端均为 **v1.9.1 (24)**、连接同一 Supabase 项目且使用同一站点账号，然后在列表下拉刷新。列表应以 Supabase 查询结果为准；若仍不一致，检查 RLS、hub/store claims 与查询范围。
