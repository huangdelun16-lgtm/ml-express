...
// 客户端网站翻译文本 (移至组件外部以提高性能并解决初始化顺序问题)
const translations = {
  zh: {
    nav: {
      home: '首页',
    ...
    ui: {
      ...
      fragile: '易碎品',
      foodDrinks: '食品和饮料',
      standardPackageDetail: '标准件（45x60x15cm）及（5KG）以内',
      overweightPackageDetail: '超重件（5KG）以上',
      oversizedPackageDetail: '超规件（45x60x15cm）以上',
      onTimeDelivery: '准时达（下单后1小时内送达）',
      urgentDelivery: '急送达（下单后30分钟内送达）',
      scheduledDelivery: '预约配送（客户指定时间）',
      selectDeliverySpeed: '请选择配送速度',
      packageInfoMismatch: '若实际物品与包裹信息不符，可能会产生计费错误',
      selectDeliveryTime: '请选择配送时间',
      selectDate: '选择日期',
      selectTime: '选择时间',
      confirmTime: '确认时间',
      cancel: '取消',
      ...
      scanToPay: '扫码支付',
      priceBreakdown: '价格明细',
      paymentWarning: '⚠️ 注意：付款后概不退款。请在付款前确认订单信息。',
      cashPayment: '现金支付',
      cashPaymentDesc: '选择现金支付，快递员上门取件时将收取费用',
      selectPaymentMethod: '选择支付方式',
      qrPayment: '二维码支付',
      underDevelopment: '开发中',
      basePrice: '基础运费',
      distanceFee: '超公里费',
      packageTypeFee: '包裹附加费',
      weightFee: '重量附加费',
      speedFee: '速度附加费',
      orderEmailSending: '正在发送订单确认邮件，请稍候...',
      orderEmailSent: '订单确认邮件已发送，请检查您的邮箱。',
      orderEmailSentDev: '开发模式：邮件未实际发送。请手动保存二维码。',
      orderFollowup: '快递员将在1小时内联系您取件。',
      packageTypeInfo: {
        title: '包裹类型说明',
        standard: '标准件（45x60x15cm）及（5KG）以内。',
        overweight: '重量超过5KG的包裹。重物需加收额外费用。',
        oversized: '尺寸超过标准规格（45x60x15cm）的大型包裹。',
        fragile: '玻璃、陶瓷等易损坏物品。需加收特殊处理费。',
        foodDrinks: '食品和饮料类。建议选择加急配送以保证新鲜。',
        document: '纸质文件类，如信函、护照、合同等。'
      }
    }
  },
  en: {
    ...
    process: {
      title: 'Our Process',
      step1: 'Order Online',
      step2: 'Doorstep Pickup',
      step3: 'Express Delivery',
      step4: 'Proof of Delivery'
    },
    tracking: {
    ...
    order: {
      ...
      mapTip: '💡 Tip: Click on the map to pin location, address will be auto-filled. You can add more details like house number.',
      mapPlaceholder: 'Enter detailed address or pick on map',
    ...
    errors: {
      ...
      dbConnectionFailed: 'Database connection failed, app running in offline mode',
      connectionTestError: 'Connection test error',
      saveCustomerFailed: 'Failed to save customer info',
      orderInfoLost: 'Order information lost, please order again',
      qrDownloaded: 'QR code downloaded and sent to customer',
      addressRequired: 'Please fill in both pickup and delivery addresses',
      packageInfoRequired: 'Please fill in package details',
      orderProcessFailed: 'Order processing failed',
      distanceCalculationFailed: 'Distance calculation failed, using default value',
      orderEmailSendFailed: 'Failed to send confirmation email. Please save QR code manually.',
      orderEmailMissing: 'Customer email not found. Please save QR code manually.'
    },
    ui: {
      packageTracking: 'Package Tracking',
      lightningDelivery: 'Express Delivery',
      secureReliable: 'Safe & Secure',
  ...
  my: {
    ...
    errors: {
      ...
      dbConnectionFailed: 'ဒေတာဘေ့စ် ချက်ဆက်မှု မအောင်မြင်ပါ၊ အက်ပ် အော့ဖ်လိုင်း မုဒ်တွင် လည်ပတ်မည်',
    ...
    ui: {
      ...
      orderEmailSentDev: 'ဖွံ့ဖြိုးတိုးတက်မှု မုဒ် - အီးမေးလ်ကို มပို့ရသေးပါ။ QR ကုဒ်ကို ကိုယ်တိုင် သိမ်းဆည်းပါ။',
            ...
            <div style={{ 
              ...
              {isLoginMode && (
                <div style={{
                  ...
                  >
                    {language === 'zh' ? '会员' : language === 'en' ? 'Member' : 'အဖွဲ့ဝင်'}
                  </button>
                  <button
                    type="button"
                    onClick={() => {
                  ...
                  >
                    {language === 'zh' ? '商家' : language === 'en' ? 'MERCHANTS' : 'ကုန်သည်'}
                  </button>
                </div>
              )}
              {/* 注册模式：显示描述文字 */}
